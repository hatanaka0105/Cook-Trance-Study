...
{
	//頂点シェーダとピクセルシェーダ、および関連するオブジェクトをシャットダウンします。
	ShutdownShader();
...
bool PBRShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	//シェーダの初期化の主な変更点の1つは、polygonLayoutです。
	//照明に使用される法線ベクトルの3番目の要素を追加します。
	//セマンティック名はNORMALで、フォーマットは通常のDXGI_FORMAT_R32G32B32_FLOATで、
	//法線ベクトルのx、y、zの3つの浮動小数点数を扱います。
	//レイアウトは、HLSL頂点シェーダへの予想入力と一致するようになりました。
	...
	XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor, XMFLOAT3 cameraPosition, float roughness, float fresnel)
{
	...
	//ここでは、カメラのバッファをロックし、その中にカメラの位置の値を設定します。
...
void PBRShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	...
	//この三角形の連打林gぬに使うシェーダーパラメータを設定
	deviceContext->VSSetShader(m_vertexShader, NULL, 0);
	deviceContext->PSSetShader(m_pixelShader, NULL, 0);

	//ピクセルシェーダーのサンプラーステートをセット
